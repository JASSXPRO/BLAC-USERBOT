...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "BLAC User"
SAVAGE_logo = "./H1M4N5HU0P/BLAC_BOT_LOGOjpg"
...
async def install(event):
    ...
    _o = o.split("\n")
    o = "\n".join(_o)
    OUTPUT = f"List of Plugins in BLAC BOT :- \n\n{o}\n\n<><><><><><><><><><><><><><><><><><><><><><><><>\nHELP:- If you want to know the commands for a plugin, do :- \n.plinfo <plugin name> without the < > brackets. \nJoin https://t.me/BLACUSERBOT_PUBLIC for help."
    if len(OUTPUT) > 69:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "cmd_list.text"
            SAVAGE_file = await bot.send_file(
                event.chat_id,
                out_file,
                force_document=True,
                allow_cache=False,
